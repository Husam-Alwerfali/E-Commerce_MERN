...
async function simulatePurchase() {
  try {
    ...
    console.log("\n=== AFTER PURCHASE ===");
    console.log(`Product: ${product.title}`);
    console.log(`Stock: ${product.stock} units (decreased by ${quantityToBuy})`);
    console.log(`Sales: ${product.salesCount} sold (increased by ${quantityToBuy})`);
    console.log(`Status: ${product.stock > 0 ? '✅ Still in Stock' : '❌ Out of Stock'}`);

    // Show all products status
    console.log("\n=== ALL PRODUCTS CURRENT STATUS ===");
    const allProducts = await productModel.find({});
    allProducts.forEach(p => {
      console.log(`${p.title}: ${p.stock} units (${p.salesCount} sold) ${p.stock === 0 ? '❌ OUT OF STOCK' : '✅'}`);
    });
